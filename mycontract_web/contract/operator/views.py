...
def counter(request):
    ...
    if types == 0 :
        ...
        contract_name = request.POST.get('contract_name')
        content = request.POST.get('content')
        print(user_name + contract_name + content)
...
def Check(request):
    ...
    response['customer'], response['start_time'], response['end_time'], response['content'] = check(contract_name)
...
def finalize(request):
    ...
    if types == 0 :
        ...
        contract_id = Contract.objects.filter(name=contract_name).first().id
        content = request.POST.get('content')
        user_id = User.objects.filter(name=request.POST.get('user_name')).first().id
        ...
        if accept == 1:
            ...
            if flag:
                ...
                for row in query_set_email:
                    email = User.objects.filter(id=row.user_id).first().email
                    send('待签订', '《%s》合同等待您签签订'%(contract_name), email)
        else :
            # the behaviour is recorded in the log
            ...
            email = User.objects.filter(id=Contract.objects.filter(name=contract_name).first().user_id).first().email
            send('重新定稿', '《%s》合同等待您重新定稿'%(contract_name), email)
        response['state'] = 0
        return JsonResponse(response)
